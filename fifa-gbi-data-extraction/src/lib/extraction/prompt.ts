...
export function buildExtractionPrompt(options: PromptOptions) {
  ...
  return [
    ...
    '   ',
    '   fifaDiscipline:',
    '   - "Association football (11-a-side)" | "Futsal" | "Beach soccer" | "Para football"',
    '   ',
    '   levelOfPlay:',
    '   - Examples: "amateur" | "semi-professional" | "professional"',
    '   - Extract as reported in the paper (not a strict list)',
    '   ',
    ...
    '   ',
    '   C. MODE OF ONSET AND CONTACT FIELDS (NUMERIC COUNTS ONLY):',
    '   - injuryModeRepetitiveGradual: Extract NUMERIC COUNT of injuries (number only, not proportion or description)',
    '   - injuryModeRepetitiveSudden: Extract NUMERIC COUNT of injuries (number only, not proportion or description)',
    '   - injuryModeAcuteSudden: Extract NUMERIC COUNT of injuries (number only, not proportion or description)',
    '   - injuryContact: Extract NUMERIC COUNT of contact injuries (number only)',
    '   - injuryNonContact: Extract NUMERIC COUNT of non-contact injuries (number only)',
    '   - injuryCumulativeRepetitive: Extract NUMERIC COUNT of cumulative repetitive injuries (number only)',
    '   - Examples: "45" NOT "45%" or "45 injuries" or descriptive text',
    '   ',
