...
Abdel handles production deployment for this project. After pushing an approved commit, check and report the GitHub/Vercel status only. Do not trigger a manual deployment, redeploy through the unrelated locally linked Vercel project, accept team invitations, or attempt another deployment workaround unless Abdel explicitly asks for that exact action.
