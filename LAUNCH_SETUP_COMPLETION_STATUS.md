...
**Script:** `setup-production-passwords.sql`

**What needs to be done:**
1. Go to [Supabase Dashboard](https://app.supabase.com)
2. Navigate to **SQL Editor** → **New query**
3. Open `setup-production-passwords.sql` and copy ALL the SQL
...
**Script:** `verify-launch-setup.sql`

**What needs to be done:**
1. After running `setup-production-passwords.sql`
2. In Supabase Dashboard > SQL Editor
3. Open `verify-launch-setup.sql` and copy ALL the SQL
4. Paste and run
...
| File | Purpose | Status |
|------|---------|--------|
| `clear-test-data.sql` | Clear test data | ✅ Already executed via Node.js script |
| `setup-production-passwords.sql` | Create passwords | ⏳ **RUN THIS NEXT** |
| `verify-launch-setup.sql` | Verify setup | ⏳ Run after password setup |
...
1. **Run `setup-production-passwords.sql` in Supabase Dashboard**
   - This creates auth.users entries with passwords
   - Takes ~30 seconds to execute

2. **Run `verify-launch-setup.sql` in Supabase Dashboard**
...
**Next Action:** Run `setup-production-passwords.sql` in Supabase Dashboard
