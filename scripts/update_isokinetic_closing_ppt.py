...
WHAT_WE_NEED_FINAL_STILL = REMOTION_OUT_DIR / "ClosingWhatWeNeedSlide-last.png"
NEXT_STEPS_VIDEO = REMOTION_OUT_DIR / "ClosingNextStepsSlide.mp4"
NEXT_STEPS_FINAL_STILL = REMOTION_OUT_DIR / "ClosingNextStepsSlide-last.png"
...
def main() -> None:
    required = [
        PPT_PATH,
        WHAT_WE_NEED_VIDEO,
        WHAT_WE_NEED_FINAL_STILL,
        NEXT_STEPS_VIDEO,
        NEXT_STEPS_FINAL_STILL,
    ]
    ...
    what_we_need_ms = ffprobe_duration_ms(WHAT_WE_NEED_VIDEO)
    next_steps_ms = ffprobe_duration_ms(NEXT_STEPS_VIDEO)

    with zipfile.ZipFile(PPT_PATH, "r") as zf:
        files = {info.filename: zf.read(info.filename) for info in zf.infolist()}

    files["ppt/media/media14.mp4"] = WHAT_WE_NEED_VIDEO.read_bytes()
    files["ppt/media/image30.png"] = WHAT_WE_NEED_FINAL_STILL.read_bytes()
    files["ppt/media/media17.mp4"] = NEXT_STEPS_VIDEO.read_bytes()
    files["ppt/media/image34.png"] = NEXT_STEPS_FINAL_STILL.read_bytes()

    files["ppt/slides/slide44.xml"] = set_video_duration_ms(
        files["ppt/slides/slide44.xml"],
        what_we_need_ms,
        "ClosingWhatWeNeedSlide.mp4",
    )
    files["ppt/slides/slide45.xml"] = set_still_description(
        files["ppt/slides/slide45.xml"],
        "ClosingWhatWeNeedSlide-last.png",
    )

    files["ppt/slides/slide49.xml"] = set_video_duration_ms(
        files["ppt/slides/slide44.xml"],
        next_steps_ms,
        "ClosingNextStepsSlide.mp4",
    )
    files["ppt/slides/_rels/slide49.xml.rels"] = swap_rel_targets(
        files["ppt/slides/_rels/slide44.xml.rels"],
        {
            "rId2": "../media/media17.mp4",
            "rId3": "../media/media17.mp4",
            "rId4": "../media/image30.png",
        },
    )
    files["ppt/slides/slide50.xml"] = set_still_description(
        files["ppt/slides/slide45.xml"],
        "ClosingNextStepsSlide-last.png",
    )
    files["ppt/slides/_rels/slide50.xml.rels"] = swap_rel_targets(
        files["ppt/slides/_rels/slide45.xml.rels"],
        {"rId2": "../media/image34.png"},
    )

    files["[Content_Types].xml"] = add_content_type_override(files["[Content_Types].xml"], 49)
    files["[Content_Types].xml"] = add_content_type_override(files["[Content_Types].xml"], 50)
    ...
    new_slide_rels = {
        "slide49.xml": f"rId{next_rel_num}",
        "slide50.xml": f"rId{next_rel_num + 1}",
    }
    ...
    slide_ids = {
        "slide49.xml": str(max_slide_id + 1),
        "slide50.xml": str(max_slide_id + 2),
    }
    rel_target_map = {
    ...
    for node in ordered:
        ...
        if target == "slides/slide45.xml":
            rebuilt.append(
                ET.Element(
                    f"{{{P_NS}}}sldId",
                    {"id": slide_ids["slide49.xml"], f"{{{R_NS}}}id": new_slide_rels["slide49.xml"]},
                )
            )
            rebuilt.append(
                ET.Element(
                    f"{{{P_NS}}}sldId",
                    {"id": slide_ids["slide50.xml"], f"{{{R_NS}}}id": new_slide_rels["slide50.xml"]},
                )
            )
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="isokinetic_closing_split_"))
